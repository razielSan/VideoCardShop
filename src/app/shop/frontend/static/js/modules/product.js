...
class Product {
  ...
  static btnPrevPagination = document.querySelector(".pagination-btn-prev");
  ...
  bindEvents() {
    ...
    // Стрелочная функция обрабатывающия нажатие на кнопки назад и вперед
    const handlePaginationEvent = (event) => {
      ...
      if (this.currentPage > liElements.length) {
        this.currentPage = 1;
      } else if (this.currentPage < 1) {
        this.currentPage = liElements.length;
      }

      this.renderProducts();
    };

    this.constructor.btnNextPagination.addEventListener(
      "click",
      handlePaginationEvent
    );
    this.constructor.btnPrevPagination.addEventListener(
      "click",
      handlePaginationEvent
    );
  }
  ...
  renderProducts() {
    ...
    productsOnPage.forEach(({ id, photo, series, prices }) => {
      ...
      liContainerElement.classList.add(
        "product",
        "item",
        "column",
        "aic",
        "js-product"
      ); // создание элемента li с классом

      liContainerElement.innerHTML = `
              ...
              <a href="#" id="${id}" class="link column aic js-link-card">
                  ...
                  <div class="product-image row jcc">
                      <img src="${photo[0]}" alt="" class="image js-image-card">   
                  </div>
                  <div class="product-description">
                      <h3 class="title js-title-card">${series}</h3>
                  </div>  
          ...
          `;
      this.container.append(liContainerElement); // добавление li к контайнеру
    });
  }

  showPagination() {
    const pagesCount = Math.ceil(this.products.length / this.productCount);
    const ul = document.querySelector(".js-pagination-list");
    for (let page = 1; page <= pagesCount; page++) {
      const li = this.renderBtn(page);
      ul.append(li);
    }

    Product.pagination.classList.remove("hidden");
  }
  ...
  updatePaginationEvent() {
    this.constructor.pagination.addEventListener("click", (event) => {
